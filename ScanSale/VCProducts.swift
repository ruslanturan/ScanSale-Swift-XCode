...
//  ScanSale
//
//  Created by Ruslan Cahangirov on 4/27/20.
//  Copyright © 2020 Ruslan Cahangirov. All rights reserved.
//

import UIKit

class VCProducts: UIViewController {
    var kinds = Data()
    var productId:Int = 0
    var screenWidth = UIScreen.main.bounds.width
    var screenHeight = UIScreen.main.bounds.height
    var kindList = UIScrollView()
    ...
    override func viewWillAppear(_ animated: Bool) {
        DispatchQueue.main.async {
            [weak self] in
            self?.productList = UICollectionView(frame: CGRect(x: 0, y: 200, width: self!.screenWidth, height: self!.screenHeight - 100), collectionViewLayout: UICollectionViewFlowLayout.init())
            self?.navigationItem.title = "პროდუქტები"
            self?.kindList.layer.backgroundColor = UIColor.white.cgColor
            self?.kindList.frame = CGRect(x: 0, y: 100, width: self!.screenWidth, height: 100)
            self?.view.addSubview(self!.kindList)
            self?.view.layer.backgroundColor = UIColor.white.cgColor
            do{
                for view in self!.kindList.subviews {
                    view.removeFromSuperview()
                }
                var x = 20
                if let jsonArray = try JSONSerialization.jsonObject(with: self!.kinds, options: .allowFragments) as? [Dictionary<String,Any>]
                {
                    for kind in jsonArray{
                        DispatchQueue.main.async {
                            ...
                            self?.kindList.addSubview(button)
                                x += btnWidth + 30
                            self?.kindList.contentSize = CGSize(width: CGFloat(Float(x + 60)), height: 100)
                        }
                    }
                } else {
                    print("bad json")
            ...
            let url = URL(string: "https://myoutlet.ge/api/products/getbykind/" + String(self!.productId))!
                    ...
                    if let jsonArray = try JSONSerialization.jsonObject(with: data, options : .allowFragments) as? [Dictionary<String,Any>]
                    {
                        ...
                        for product in jsonArray{
                            ...
                            DispatchQueue.main.async {
                                ...
                                productItem.backgroundColor = UIColor.white
